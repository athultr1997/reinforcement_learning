...
class MC_Agent:
	...
	def MC_exploring_starts(self):
		...
		for i in range(500000):
			...
			env = Environment(self)
			states, actions, rewards = env.play1(initial_state, initial_action)
			G = 0
...
class Environment:
	...
	def generate_reward(self, terminal_state):
	...
	def dealer_play(self, action=None):
		...
		self.print_game_status(player_sum=self.player_sum, dealer_sum=self.dealer_sum, dealer_card=card, player_card=None)
		self.turns += 1
		return card, action
	...
	def player_play(self, action=None):
		...
		self.print_game_status(player_sum=self.player_sum, dealer_sum=self.dealer_sum, dealer_card=None, player_card=card)
		self.turns += 1
		return card, action
	...
	def play(self, initial_state=None, initial_action=None):
		...
		if initial_state==None or initial_action==None:
			self.showing_card, action = self.dealer_play()
			self.dealer_play()

			self.player_play()
			self.player_play()
		else:
			print('initial_state = ', initial_state, '; initial_action = ', initial_action)
			self.player_sum = initial_state[0] + 12
			self.showing_card = initial_state[1] + 1
			self.player_usable_ace = initial_state[2]			
			self.turns = 4

			reward = self.generate_reward(terminal_state=False)			
			actions.append(initial_action)
			rewards.append(reward)
		
		while True:															
			print('Player: action=',action,'reward=', reward)
			state = (self.player_sum-12, self.showing_card-1, self.player_usable_ace)			

			if reward==1 or reward==-1 or action==self.player.actions['stick']:
				break
			else:
				if state[0]>=0:
					states.append(state)											
		...
		action = None		
		while True:			
			if reward==1 or reward==-1 or action==self.dealer.actions['stick']:				
				break
			else:				
				card, action = self.dealer_play()				
			reward = self.generate_reward(terminal_state=False)

		reward = self.generate_reward(terminal_state=True)						
		rewards[-1] = reward
		return states, actions, rewards

	def play1(self, initial_state=None, initial_action=None):
		self.reset()		
		card = None
		action = None
		reward = None
		...
		self.player_sum = state[0] + 12
		self.showing_card = state[1] + 1 # inlcude 11
		self.dealer_sum += self.showing_card
		self.player_usable_ace = state[2]
